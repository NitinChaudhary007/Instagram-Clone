...
import { FaBookmark, FaComment, FaGreaterThan, FaHeart } from "react-icons/fa";
...
const Post = ({ post }) => {
  ...
  const likeDislikeHandler = async () => {
    try {
      ...
      if (res.data.success) {
        setIsLiked(!isLiked);
        const updatedLikes = isLiked ? postLike - 1 : postLike + 1;
  ...
  const deletePostHandler = async () => {
    try {
      ...
      if (res.data.success) {
        const updatedPostData = posts.filter((item) => {
      ...
      <div className="flex items-center justify-between my-2">
        ...
        <div className="flex gap-3">
          ...
          <FaComment
            ...
            className="cursor-pointer hover:text-gray-600"
          />
          <FaGreaterThan className="cursor-pointer hover:text-gray-600" />
        </div>
        <FaBookmark
